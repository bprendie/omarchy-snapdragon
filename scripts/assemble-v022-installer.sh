...
set -euo pipefail
cd "$(dirname "$0")/.."
[[ $# == 2 ]] || { echo 'Usage: assemble-v022-installer.sh REPOSITORY_BUILD_NAME PINNED_PRIMARY_FINGERPRINT' >&2; exit 1; }
name=$1 key=$2
[[ $name =~ ^[a-zA-Z0-9_-]+$ && $key =~ ^[A-F0-9]{40}$ ]]
repo=$PWD/build/$name
stage=build/snapdragon-v022
iso=omarchy-snapdragon-v0.2.2.iso
...
sha256sum -c manifests/asus-a16-audio-package.sha256
[[ -f $stage/diagnostic.img && -f $stage/boot-images/gpt_part2_efi.img ]]
patch --fuzz=0 -o "$stage/phases_impl.py" "$stage/root/usr/share/omarchy-iso/orchestrator/phases_impl.py" < profiles/snapdragon/installer-kernel-update-v022.patch
date -u +%FT%TZ > "$stage/integration-started"
sed -e 's/Omarchy Snapdragon v0\.1\.2 -/Omarchy Snapdragon v0.2.2 -/' -e 's@ThinkPad / HP / ASUS A14 (experimental)@ThinkPad / HP / ASUS A14 / A16 (experimental)@' \
...
docker run --rm --network none -v "$PWD/build:/work" oma-snap-builder:local bash -ec '
  root=/work/snapdragon-v022/root
  install -m644 /work/snapdragon-v022/grub-live.cfg /work/snapdragon-v022/iso/boot/grub/grub.cfg
  install -m644 /work/snapdragon-v022/phases_impl.py "$root/usr/share/omarchy-iso/orchestrator/phases_impl.py"
  ...
  cp /work/asus-a16-audio-package/oma-snap-audio-asus-a16-0.2.2-1-any.pkg.tar.xz "$offline/"
...
' bash "$name" "$channel"
xz -dc build/asus-a16-audio-package/oma-snap-audio-asus-a16-0.2.2-1-any.pkg.tar.xz |
  docker run --rm -i --network none -v "$PWD/build:/work" oma-snap-builder:local \
    tar -xf - -C /work/snapdragon-v022/root --exclude .PKGINFO --exclude .BUILDINFO --exclude .MTREE
repo-add "$stage/root/var/cache/omarchy/mirror/offline/oma-snap-local.db.tar.gz" \
  ...
  build/asus-a16-audio-package/oma-snap-audio-asus-a16-0.2.2-1-any.pkg.tar.xz
...
docker run --rm --network none -v "$PWD/build:/work" oma-snap-builder:local \
  install -m644 /work/snapdragon-v022/pacman-live.conf /work/snapdragon-v022/root/etc/pacman.conf
# Only the public repository key enters the live image's pacman trust store.
docker exec oma-snap-root pacman-key --gpgdir /output/snapdragon-v022/root/etc/pacman.d/gnupg --init
docker exec oma-snap-root pacman-key --gpgdir /output/snapdragon-v022/root/etc/pacman.d/gnupg \
