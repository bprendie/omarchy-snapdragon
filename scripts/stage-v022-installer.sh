...
destination=build/snapdragon-v022
[[ -f $iso && ! -e $destination ]]
...
docker run --rm --network none -v "$PWD/build:/work" oma-snap-builder:local \
  unsquashfs -processors 2 -no-progress -d /work/snapdragon-v022/root \
  /work/snapdragon-v022/iso/oma_snap/aarch64/airootfs.sfs
printf '%s  %s\n' c622aff8c5e413e71d26009ad09192b4dc31e50a050198c5e2d7235bd39ddb5d "$iso" > "$destination/base-iso.sha256"
echo 'Staged verified v0.1.2 base for v0.2.2; no updated image assembled'
