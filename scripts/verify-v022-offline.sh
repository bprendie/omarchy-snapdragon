...
set -euo pipefail
cd "$(dirname "$0")/.."
stage=${1:-snapdragon-v022}
check_name=${2:-v022-offline-check}
tools_version=${3:-0.2.2-1}
[[ $stage =~ ^[a-zA-Z0-9_-]+$ && $check_name =~ ^[a-zA-Z0-9_-]+$ && $tools_version =~ ^[0-9.]+-[0-9]+$ ]]
root=build/$stage/root
check=build/$check_name
[[ ! -e $check ]]
...
GPGDir = /output/$stage/root/etc/pacman.d/gnupg
SigLevel = Required DatabaseOptional
[oma-snap]
SigLevel = PackageRequired DatabaseRequired TrustedOnly
Server = file:///output/$stage/root/var/cache/oma-snap/repository
[oma-snap-local]
SigLevel = Never
Server = file:///output/$stage/root/var/cache/omarchy/mirror/offline
[offline]
Server = file:///output/$stage/root/var/cache/omarchy/mirror/offline
CONF
docker exec oma-snap-root bash -ec '
  ...
  pacman --config "/output/$1/pacman.conf" --dbpath "/output/$1/db" \
...
grep -Fxq "oma-snap-kernel-tools-$tools_version-aarch64.pkg.tar.xz" "$check/packages.txt"
